...
_VAL_IMAGES_ ='/media/Disk3/Datasetdir/landmark_dataset/images/'
...
pic_lm_in = open("/media/nvme/Datasets/Celeba/lm_dict.pickle", "rb")
...
lut_celeba_lm = pickle.load(pic_lm_in)
...
_SAVE_DIR_ = '/media/nvme/TFmodels/celeba/'
...
aug = iaa.SomeOf((0, None), [
    ...
    iaa.Affine(scale=(0.8, 1.2), translate_percent={"x": (-0.2, 0.2), "y": (-0.2, 0.2)}, rotate=(-10, 10), shear=(-16, 16), mode=['edge'])
    #iaa.pad()
...
def make_celebagt(inp):
    ...
    gt_lms = np.zeros(10, np.float32)
    return gt_val, gt_lms
    gt_val = lut_celeba[inp + '.jpg']
    gt_val = gt_val.astype(np.float32)
    gt_lms = lut_celeba_lm[inp + '.jpg']
    gt_lms = gt_lms.astype(np.float32)
    #gt_val = np.concatenate((gt_val, gt_lms), axis=0)
...
def get_imageandtags(path):
    ...
    GT_data, gt_lms = make_celebagt(name)
    image = cv2.imread(path.decode())
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    image = cv2.resize(image, (_IMAGE_WIDTH_,_IMAGE_HEIGHT_), 0)
    ...
    if _TRAINING_:
        ...
        gt_lms = np.reshape(gt_lms, [-1])


    GT_data = np.concatenate((GT_data, gt_lms), axis=0)
    return image, GT_data
...
def get_image_tags_points(path):
    ...
    lm_image -= 0.5
    lm_image *= 2.0
    return image, GT_data, lm_image, lmpts, mean_pts


def decode_img_lmpts(image1):
    image, gt_data, lm_image, lmpts, mean_pts = tf.py_func(get_image_tags_points, [image1],
                                                           (tf.uint8, tf.float32, tf.float32, tf.float32, tf.float32))
    ...
    lm_image.set_shape([_PAD_WIDTH_, _PAD_WIDTH_, 3])
...
def draw_landmarks(image, pts, trg):
            pts = pts
...
def main():
    ...
    with tf.Session() as session:
        ...
        if _TRAINING_:
            Simple_DNN.Train_Iter(iterations=_ITERATIONS_, save_iterations=_SAVE_INTERVAL_, restore=_RESTORE_,
                                  log_iteration=5)
        ...
        for i in range(len(Val_images)):
            preds, gt, image, lmpts, gtlmpts = Simple_DNN.Predict(session=session)
            lmpts = np.reshape(lmpts, [-1, 2])
            gtlmpts = np.reshape(gtlmpts, [-1, 2])

            image = image[0]
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            image = draw_landmarks(image, lmpts, lmpts)
            ...
            if gt[0] == True:
                ...
                prefix = 'Not_Accesory'
            image = ((image + 1) / 2) * 255
            cv2.imwrite('/media/Disk3/Projects/PersonalGit/Tensorflow_Playground/Super_TF/out/out.jpg', image)
            if rtr2[0] == gt[0]:
