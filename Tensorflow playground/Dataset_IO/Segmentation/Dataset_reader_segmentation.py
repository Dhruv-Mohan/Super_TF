...
class Dataset_reader_segmentation(Dataset_reader, Dataset_config_segmentation):
    ...
    def __init__(self, filename=None, epochs=100, num_classes=1):
        ...
        with tf.name_scope('Dataset_Segmentation_Reader') as scope:
            ...
            self.flip_prob = tf.Variable(tf.random_uniform(shape=[1], maxval=1, dtype=tf.float32))
            self.crop_prob = tf.Variable(tf.random_uniform(shape=[1], maxval=1, dtype=tf.float32))
            self.crop_val = tf.Variable(tf.random_uniform(shape=[1], minval=1.1, maxval=1.25, dtype=tf.float32))
            ...
            self.images , self.masks , self.mask_weights = self.batch_inputs()
    ...
    def batch_inputs(self):
        ...
        images,masks ,mask_weights= tf.train.shuffle_batch([image, mask, mask_weight], batch_size=self.batch_size, num_threads=4, capacity=3, min_after_dequeue=2)
    ...
    def next_batch(self, batch_size=1, sess=None):
        with tf.name_scope('Batch_geter') as scope:
            if sess is None :
                ...
                self.sess = sess
            images, masks, mask_weights= self.sess.run([self.images, self.masks, self.mask_weights], feed_dict={self.batch_size : batch_size})
            return images, masks, mask_weights
