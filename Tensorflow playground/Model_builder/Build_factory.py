        ...
        with tf.name_scope('LeNeT'):
            ...
            with Builder(**self.kwargs) as lenet_builder:
                input = tf.placeholder(tf.float32, \
                    shape=[self.kwargs['Batch_size'], self.kwargs['Image_width']*self.kwargs['Image_height']*self.kwargs['Image_cspace']])
                output_ = tf.placeholder(tf.float32, shape=[self.kwargs['Batch_size'], self.kwargs['Classes']])
